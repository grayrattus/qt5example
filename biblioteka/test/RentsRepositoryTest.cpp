...
BOOST_AUTO_TEST_CASE(PodstawowyKonstruktor) {
    ...
    rentsRepository.create(rent1);
    rentsRepository.create(rent2);
    rentsRepository.create(rent3);
    
    auto allRents = rentsRepository.getAll();
	BOOST_CHECK_EQUAL(allRents.size(), 3); 
}
